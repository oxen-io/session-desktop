...
test_1.test('Create User', async () => {
    ...
    // Create User
    const user = await new_user_1.newUser(window, 'testuser');
    await window.click('[data-testid=leftpane-primary-avatar]');
    await Promise_1.sleepFor(100);
    //check username matches
    test_1.expect(await window.innerText('[data-testid=your-profile-name]')).toBe(user.userName);
    //check session id matches
    test_1.expect(await window.innerText('[data-testid=your-session-id]')).toBe(user.sessionid);
    // Exit profile module
