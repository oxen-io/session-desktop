...
function throwIfNoFirstInstance(instanceToCastIfValid, pageToCastIfValid) {
...
async function createAppInstance(MULTI) {
    // Launch Electron app.
    process.env.NODE_ENV = NODE_ENV;
    process.env.NODE_APP_INSTANCE = `${MULTI}`;
    const instance = await playwright_1._electron.launch({
        args: ['main.js'],
    });
...
async function killAppInstance(appInstance) {
    // Kill Electron app.
    if (appInstance) {
...
//   });
// });
//# sourceMappingURL=sample_test.spec.js.map