...
import { initOutgoingMessage, MessageModelType } from '../../models/messageType';
...
export async function addUpdateMessage(
  ...
  // for this case we do not care about the source. 
  // the person who made the change is present on the group_update field itself.
  const args = {
  ...
  const message = await convo.addMessage(initOutgoingMessage(args));
