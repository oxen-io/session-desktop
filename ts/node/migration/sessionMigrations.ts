...
function updateToSessionSchemaVersion31(currentVersion: number, db: BetterSqlite3.Database) {
  ...
  db.transaction(() => {
    ...
    try {
      ...
      MIGRATION_HELPERS.V31.writeConfigDumps(
        db,
        targetVersion,
        publicKeyHex,
        'UserConfig',
...
function updateToSessionSchemaVersion33(currentVersion: number, db: BetterSqlite3.Database) {
  ...
  db.transaction(() => {
    ...
    const userConfigWrapperDump = MIGRATION_HELPERS.V33.fetchConfigDumps(
      db,
      targetVersion,
      publicKeyHex,
      'UserConfig'
    ...
    if (isNil(blindedReqEnabled)) {
      ...
      MIGRATION_HELPERS.V33.writeConfigDumps(
        db,
        targetVersion,
        publicKeyHex,
        'UserConfig',
    ...
    sqlNode.createOrUpdateItem(
      {
        id: SettingsKey.hasBlindedMsgRequestsEnabled,
        value: blindedReqEnabled,
      },
      db
    );
